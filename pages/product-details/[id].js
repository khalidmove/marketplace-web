...
function ProductDetails(props) {
  ...
  const [getComboProductList, setGetComboProductList] = useState([]);
  ...
  const getProductById = async () => {
    ...
    Api("get", url, "", router).then(
      (res) => {
        ...
        getProductByComboId(res.data._id);
  ...
  const getProductByComboId = async (product_id) => {
    props.loader(true);
    Api(
      "get",
      `getProductByComboId/${product_id}`,
      "",
      router
    ).then(
      (res) => {
        ...
        console.log("res================>", res);
        // const sameItem = res?.data?.filter((f) => f._id !== router?.query?.id);
        setGetComboProductList(res.data);
